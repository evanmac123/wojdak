...
class Wpsms_Base_Network {
	...
	public function ago( $time )
	{
		...
		if ( $time < ($now - $yesterday)) {
			...
			$post_time = $this->local_date_i18n( 'F j, Y', $time);
		}
    ...
    /**
     * Get a local time via a timestamp
     *
     * @link    http://wordpress.stackexchange.com/a/135049/48162
     * @since   1.2.1
     * @param   string    $format       the date to ouput the timestamp in
     * @param   string    $timestamp    the unix timestamp
     * @return  string                  the formatted time
     */
    public function local_date_i18n($format, $timestamp) {
        $timezone_str = get_option( 'timezone_string' ) ? get_option( 'timezone_string' ) : 'UTC';
